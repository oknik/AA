...
public:
    static Tree* createTree() {
        auto tree = new Tree();
        if (tree && tree->initWithFile("Tree.png")) {
            tree->autorelease();
            tree->hp = 100;
            return tree;
        }
        CC_SAFE_DELETE(tree);//cocos中的宏，可以安全删除
        return nullptr;
    }
};
...
public:
    static BigTree* createBigTree() {
        ...
        if (bigTree && bigTree->initWithFile("BigTree.png")) {
            bigTree->autorelease();
            bigTree->hp = 200;
            return bigTree;
        }
        CC_SAFE_DELETE(bigTree);//cocos中的宏，可以安全删除
        return nullptr;
    }
};
...
public:
    static Ice* createIce() {
        auto ice = new Ice();
        if (ice && ice->initWithFile("Ice.png")) {
            ice->autorelease();
            ice->hp = 100;
            return ice;
        }
        CC_SAFE_DELETE(ice);//cocos中的宏，可以安全删除
        return nullptr;
    }
};
...
public:
    static BigIce* createBigIce() {
        ...
        if (bigIce && bigIce->initWithFile("BigIce.png")) {
            bigIce->autorelease();
            bigIce->hp = 200;
            return bigIce;
        }
        CC_SAFE_DELETE(bigIce);//cocos中的宏，可以安全删除
        return nullptr;
    }
};
...
public:
    static Icecream* createIcecream() {
        ...
        if (icecream && icecream->initWithFile("Icecream.png")) {
...
public:
    static House* createHouse() {
        ...
        if (house && house->initWithFile("House.png")) {
            house->autorelease();
            house->hp = 300;
            return house;
        }
...
public:
    static House* createStone() {
        ...
        if (stone && stone->initWithFile("Stone.png")) {
            stone->autorelease();
            stone->hp = 300;
            return stone;
        }
