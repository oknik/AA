...
/*====================创建炮台选择界面=========================*/
    ...
    pickCannon2->setScale(0.6f); //重新调整大小
    ...
    pickCannon3->setScale(0.6f); //重新调整大小
    ...
    listener->onTouchBegan = [blank, pickCannon1, pickCannon2, pickCannon3, validArea, this](Touch* touch, Event* event) 
        ...
        if (validArea.containsPoint(touchPos)) { //若在范围内，显示炮台选项
            ...
            pickCannon2->setVisible(false);
            pickCannon3->setVisible(false);
            AudioEngine::play2d("Wrong.mp3", false);//若不在范围内，播放错误音效
            return false;
        }
    };
...
void FirstMapScene::MonsterWave(const int type, const float blood, const int count, const float inr)
{
    if (type == NORMALMONSTER)
    {
        for (int i = 0; i < count; i++)
        {
            ...
            //碰撞体的实现
            //auto contactListener = EventListenerPhysicsContact::create();   //物理碰撞监听器对象
            //contactListener->onContactBegin = CC_CALLBACK_1(Monster::onContactBegin, monster);  //为监听器设置回调函数
            //_eventDispatcher->addEventListenerWithSceneGraphPriority(contactListener, this);
            //将碰撞监听器注册到场景的事件调度器中，使其能够接收并处理物理碰撞事件。
        }
    }
    if (type == FASTMONSTER)
    {
...
void FirstMapScene::Wave1()
{
    scheduleOnce([this](float dt)
        {
            MonsterWave(FASTMONSTER, 70, 10, 1.f); // 或者执行其他产生怪物的逻辑
            schedule([this](float dt)
                {
                    if (checkWave(dt))
                    {
...
void FirstMapScene::Wave2()
{
    scheduleOnce([this](float dt)
        {
            MonsterWave(NORMALMONSTER, 100, 10, 1.f); // 或者执行其他产生怪物的逻辑
            schedule([this](float dt)
                {
                    if (checkWave(dt))
                    {
...
void FirstMapScene::Wave3()
{
    scheduleOnce([this](float dt)
        {
            MonsterWave(NORMALMONSTER, 100, 20, 1.f); // 或者执行其他产生怪物的逻辑
            schedule([this](float dt)
                {
                    if (checkWave(dt))
                    {
...
void FirstMapScene::Wave4()
{
    scheduleOnce([this](float dt)
        {
            MonsterWave(BIGMONSTER, 70, 2, 3.f); // 或者执行其他产生怪物的逻辑
            schedule([this](float dt)
                {
                    if (checkWave(dt))
                    {
